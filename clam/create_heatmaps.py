...
import numpy as np
import sys
# sys.path.append(r"D:\project\SLFCD1")
...
if __name__ == '__main__':
    ...
    if data_args.process_list is None:
        if isinstance(data_args.data_dir, list):
            ...
            slides = sorted(os.listdir(data_args.data_dir + "/data"))
    ...
    for i in range(len(process_stack)):
        ...
        if isinstance(data_args.data_dir, str):
            slide_path = os.path.join(data_args.data_dir, 'data', slide_name)
        ...
        for sample in samples:
            if sample['sample']:
                ...
                print(sample_results['sampled_coords'])
        ...
        if os.path.isfile(os.path.join(r_slide_save_dir, heatmap_save_name)):
            ...
            heatmap, original_image = drawHeatmap(scores, coords, slide_path, wsi_object=wsi_object, cmap=heatmap_args.cmap, alpha=heatmap_args.alpha, use_holes=True, binarize=False, vis_level=-1, blank_canvas=False,
                                  thresh=-1, patch_size=vis_patch_size, convert_to_percentiles=True,
                                  k=sample_args.samples[0]['k'])
