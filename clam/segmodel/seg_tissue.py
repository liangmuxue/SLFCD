...
INPUT_PATH = r'/home/bavon/datasets/wsi/ais/org/'
SAVE_PATH = r'/home/bavon/datasets/wsi/ais/org/10_mask'
SAVE_COM_PATH = r'/home/bavon/datasets/wsi/ais/org/10_mask_concat'
...
def piplineone(img_path):
    img = get_img(img_path)  # [::2,::2,:]
    img = np.pad(img, ((224, 224), (224, 224), (0, 0)), mode='reflect')
    mask = gen_mask(model, img)
    ostu_mask = hw.ostu_seg_tissue(img)
    mask = mask * (ostu_mask > 0)
...
#model = tm.resnet34(pretrained=False)
#model.load_state_dict(torch.load('resnet34-b627a593.pth', map_location="cuda:1"))
#model.fc = nn.Linear(512, 2)
#model = model.to("cuda:1")
#model.eval()
#pretrained_model = torch.load('epoch-30.pth', map_location="cuda:1")  # 导入预训练权重
