...
class WholeSlideImage(object):
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            # save mask data
            # asset_dict["masks"] = self.mask_data
            if len(asset_dict) > 0:
                if init:
    ...
    def visHeatmap(self, scores, coords, vis_level=-1,
        ...
        for idx in range(len(coords)):
            ...
            if score >= threshold:
                ...
                # 将颜色块添加到图像上
                ax.imshow(color_block, extent=[coord[0], coord[0] + patch_size[0], coord[1], coord[1] + patch_size[1]],
                          alpha=alpha)
        ...
        # 添加颜色条
        cbar = plt.colorbar(ax.imshow(color_block), ax=ax, orientation='vertical')
        ...
        img = Image.fromarray(fig.canvas.buffer_rgba())
