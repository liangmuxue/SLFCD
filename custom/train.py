...
def train_epoch(summary, summary_writer, cnn, model, loss_fn, optimizer,
    ...
    for step in range(steps):
        ...
        predicts = (probs >= 0.5).to(device)

        acc_data = (predicts == target_train).type(torch.float).sum().data * 1.0 / batch_size
        loss_data = loss.data
...
def valid_epoch(summary, model, loss_fn,
    ...
    for step in range(steps):
        ...
        acc_data = (predicts == target_valid).type(torch.float).sum().data * 1.0 / batch_size
        loss_data = loss.data
